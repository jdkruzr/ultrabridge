...
Last verified: 2026-08-14 (reMarkable file management: `internal/source/remarkable` gained server-authored hashtree mutations (`treewrite.go` — byte-exact index/composite hashing, generation-CAS root commits) behind `Source.{UploadDocument,DownloadDocument,DeleteDocument,CreateFolder,MoveNode,RenameNode}`; surfaced via the `service.RemarkableFileManager` seam on the reMarkable Files tab (upload/download/delete/rename/move/new-folder) and `/api/v1/remarkable/{documents,folders}`. Delete = move to the tablet's trash; trashed docs drop out of UB listing/search. Prior: SyncModel + Settings IA: `internal/source` gained `SyncModel`/`SyncModelFor` — a typed per-source-type sync-semantics descriptor surfaced as `sync_model` on `GET /api/sources` and as Unicode-glyph banners (⇅/⬇) on the Files tabs; Settings split into four deep-linkable groups `/settings/{devices,ai,integrations,system}` (legacy `/settings` 303s to devices), with the Devices group rendering uniform per-source sections. Prior: ForestNote sync device management — /sync/v1 optional `device_name` envelope field; Settings "Sync Devices" card + /api/v1/sync/{devices,compact}; prune = cleanup-only delete of the sync_cursors row, spec §4.3)
