...
Last verified: 2026-08-14 (**reMarkable file management seam**: new `RemarkableFileManager` interface (note.go, next to `RemarkableReader`) — Upload/Download/Delete/CreateFolder/Move/Rename over server-authored hashtree mutations; `*remarkable.Source` satisfies it, wired via `SetRemarkableFileManager` (nil = feature absent, `HasRemarkableFileManager()` gates web/API). `NoteService` gained `UploadRemarkableDocument` / `DownloadRemarkableDocument` (rc, filename, contentType; filename sanitized service-side) / `DeleteRemarkableDocument` (move to tablet trash) / `CreateRemarkableFolder` / `MoveRemarkableNode` / `RenameRemarkableNode`. Write-path sentinels are aliased at the service boundary (`ErrRemarkable{NotFound,NoPayload,UnsupportedFile,ParentNotFound,NotAFolder,FolderNotEmpty,NoHashTree,TreeConflict}` = the rmsource errors) so web maps status codes without importing the source adapter. `RemarkableEntry` gained `FileType`/`CanDownload`; `RemarkableDocumentDetail` gained `FileType`/`DownloadAvailable` (detail now resolves `RenderDocument` once for both render and download availability); `RemarkableDocument` gained `file_type`. Prior: **ForestNote durable "done"**: `ForestNoteQueueStatus` gained `Indexed`, populated by `GetProcessorStatus` from the new `ForestNoteReader.CountIndexedPages` (best-effort — a failed count logs and leaves 0 rather than dropping the block). `Processed` is in-memory and monotonic since process start, so it can only ever be a this-session figure; `Indexed` is the count that survives a restart. Plus **operator device labels**: `SyncDeviceService.RenameSyncDevice` + `RemarkableDeviceService.RenameDevice` set an operator label stored apart from the device-reported name (blank clears; `ErrSyncDeviceNotFound` / new `ErrRemarkableDeviceNotFound` on an unknown id). `SyncDevice`/`RemarkableDevice` gained `Label` plus a `DisplayName()` method — label wins, else the reported name, else `""` — so the precedence rule has one definition instead of being re-derived per template. `devicelabel.go` holds the shared `normalizeOperatorLabel` (trim + `MaxOperatorLabelLen` = 128 runes), deliberately separate from `syncsvc.normalizeDeviceName`: that one cleans an untrusted wire envelope, this one cleans operator input. Both admin seams (`ForestNoteSyncAdmin`, `RemarkableAdmin`) gained `SetDeviceLabel`. Prior: new **`SyncDeviceService`** (`syncdevice.go`): ForestNote sync device management — `ListSyncDevices` (SyncDevice views over `syncstore.DeviceRow`), `PruneSyncDevice` (ErrSyncDeviceNotFound on miss), `CompactNow` (SyncCompactResult); wraps the `ForestNoteSyncAdmin` seam `*forestnote.Source` satisfies; `NewSyncDeviceService(nil)` returns nil so web gates the card/routes on it. Prior: NoteService.GetNotePages → []NotePageView typed page content backing the in-tab note detail grid (web); NoteService.RenderSupernotePage renders an absolute .note via go-sn directly, bypassing RenderPage's isBooxPath heuristic — for digest source pages that may render with no filesystem Supernote source configured; DigestService.GetDigest + DigestView detail fields (SourcePath/SourceType/HandwriteInnerName/NotePage) for the digest detail page; TaskService write surface for URL/Priority/Categories/Comment + ForestNote provenance + hard-purge; PurgeCompleted now returns (int64, error); PurgeDeleted now returns (purged, skipped, error); ForestNoteReprocessor.Status + EmbeddingJobStatus.ForestNote; SearchService.Search gained explicit `limit` arg with service-side default/ceiling clamp)
