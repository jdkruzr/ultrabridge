...
| POST | `/settings/save` | `handleSettingsSave` | Save config changes. Routes by hidden `section` field: `supernote`, `general`, `boox`, `ub-spc` (UB-as-SPC server config — all restart-required; secret fields keep current value when left blank), `sync` (ForestNote device-sync: SyncEnabled/SyncBatchLimit → appconfig, OCR-prompt → settings table, **and the relay-log compaction checkbox written THROUGH to the ForestNote source row's `config_json` via `setSourceConfigBool` — not appconfig, since the source reads `config_json` at Start; `forestNoteSourceRow`/`sourceConfigBool` prefill it, gated on a ForestNote source existing; restart-required**). NB: the SyncBatchLimit field writes appconfig only, which is a no-op for an already-seeded source row (legacy-seed-only) — compaction deliberately does NOT copy that and merges into the row, preserving `batch_limit`/interval/horizon. |
...
| POST | `/settings/sync-devices/prune` | `handleSyncDevicePrune` | Delete a ForestNote sync device's registry row (form `site_id`, ULID-validated). Cleanup only: authored ops stay; a live device re-registers on next sync. 404 when no SyncDeviceService wired; HX re-renders settings, non-HX 303 → `/settings#sync-devices`. |
| POST | `/settings/sync-devices/compact` | `handleSyncDeviceCompact` | Run one relay-log compaction pass on demand (NOT gated on the periodic compaction checkbox — the press is the opt-in). HX re-renders settings with a `SyncCompactResult` flash. |
